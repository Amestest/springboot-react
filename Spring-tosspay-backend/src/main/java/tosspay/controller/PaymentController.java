...
/***
 ...
 ResponseEntity(Response = 응답 / Http를 상속받아서 Http 기능에 응답에 대한 기능을 추가로 설정한 Entity) : 사용자가 요청한 응답을 개발자가 다시 사용자한테 전달할 때 사용 
 ...
 RequestEntity<String이면 String, Integer면 Integer, 여러값이면?, 모르겠으면 비워두기> req = new RequestENtity<>("요청본문",headers);
 RequestEntity<String> req = new RequestENtity<>("요청본문",headers, HttpMethod.Post, url);
 
 차이점 요약하기
 	   클래스      		 상속관계					주요사용목적				추가 정보
