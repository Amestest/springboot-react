...
function UserInfo(){
    ...
    useEffect(()=> {
        ...
        //String redirectUrl = "http://localhost:3000/userinfo?access_token=" + accessToken;
        //자바에서는 userinfo?access_token=" 뒤에 + 를 붙여 변수를 사용했찌만
        //자바스크립트에서는 ``를 사용하여 const accessToken=a.get('access_token');
        //가져옴
        ...
        //.then((res)=> {  (res) 를 막아버리면 => 이후로는 res 선언되지 않은
        // 지역변수명이 되기 때문에 res를 찾을수 없는건
        .then(response => {
    ...
    return(
        ...
        <div>
            ...
            <img src={userInfo.response.profile_image} disabled style={{height:'100px',width:'100px'}}/>
