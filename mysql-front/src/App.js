...
useEffect(()=>{
  모든유저보기(); //users = 유저목록에 유저가 추가되거나 삭제되는 일이 발생하면 모든 유저 다시보기
},[/*users*/]); // [ ] users가 들어이씩 때문에 유저목록에 유저가 추가되거나 삭제될 경우 유저목록 새로고침
  /*
  const 모든유저보기 = () => {
 ...
 /*
  ...
  // async = 기능 좀 실행해보자 await = 일단기다려 내가 값 좀 가져와볼게
  const 모든유저보기 = async () =>{
...
/*
    ...
    setUser(새로운 유저목록)에 포함시키고
    id(삭제하고자하는 유저아이디)와 user.id가 일치하는 아이디는 삭제한 다음
...
const updateUser = async (user) => {
  await axios.put('/user',user); //putmapping/user로 주소값이 설정된 수정하는 주소 연결
  setUsers(users.map(u => (u.id === user.id ? user:u)));
  // 수정한 유저의 id값이 일치하는지 확인하고, id값이 일치하지 않다면 기존에 있떤 유저 정보로 수정하지 않고 전달
}
