...
public class DBConfig {
	...
	public SqlSessionFactory sessionFactory(DataSource dataSource) throws Exception {
		...
		// 나중에 본인의 dto 패키지명 변경 dataBase 작성한 컬럼값과 dto에 작성한 변수명을 대조
		
		//우리가 나중에 컬럼명을 dto에서 카멜케이스나 dto 용법으로 작성했을 때 작성값 설정
	...
	public DataSourceTransactionManager dataSourceTransactionManager(DataSource ds) {
		...
		// 저장을 안해서 안봉니느 상황이라 여기지 않고, 코드 상이나 흐름에 문제가 있다고 생각할 수 잇기 때문에 commit 진행 매니저 생성
	}
}
